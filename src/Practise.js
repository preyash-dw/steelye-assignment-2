function highlightHTMLContent(htmlContent, plainText, plainTextPositions) {
    // Helper function to convert plain text positions to HTML content positions
    function getHTMLIndexFromPlainTextIndex(plainTextIndex) {
      const textBefore = plainText.substring(0, plainTextIndex);
      return htmlContent.indexOf(textBefore);
    }
  
    // Loop through each plain text position and update it to HTML content position
    const htmlTextPositions = plainTextPositions.map(({ start, end }) => ({
      start: getHTMLIndexFromPlainTextIndex(start),
      end: getHTMLIndexFromPlainTextIndex(end)
    }));
  
    // Create a new plain text string with the appropriate content highlighted
    let highlightedText = plainText;
    htmlTextPositions.forEach(({ start, end }) => {
      highlightedText =
  ...
  const highlightedPlainText = highlightHTMLContent(htmlContent, plainText, plainTextPositions);
  console.log(highlightedPlainText);
